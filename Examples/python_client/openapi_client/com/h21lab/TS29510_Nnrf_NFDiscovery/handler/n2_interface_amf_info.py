# coding: utf-8

"""
    NRF NFDiscovery Service

    NRF NFDiscovery Service. © 2020, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC). All rights reserved.   # noqa: E501

    The version of the OpenAPI document: 1.1.0.alpha-4
    Generated by: https://openapi-generator.tech
"""


import pprint
import re  # noqa: F401

import six

from openapi_client.configuration import Configuration


class N2InterfaceAmfInfo(object):
    """NOTE: This class is auto generated by OpenAPI Generator.
    Ref: https://openapi-generator.tech

    Do not edit the class manually.
    """

    """
    Attributes:
      openapi_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    openapi_types = {
        'ipv4_endpoint_address': 'list[str]',
        'ipv6_endpoint_address': 'list[Ipv6Addr]',
        'amf_name': 'str'
    }

    attribute_map = {
        'ipv4_endpoint_address': 'ipv4EndpointAddress',
        'ipv6_endpoint_address': 'ipv6EndpointAddress',
        'amf_name': 'amfName'
    }

    def __init__(self, ipv4_endpoint_address=None, ipv6_endpoint_address=None, amf_name=None, local_vars_configuration=None):  # noqa: E501
        """N2InterfaceAmfInfo - a model defined in OpenAPI"""  # noqa: E501
        if local_vars_configuration is None:
            local_vars_configuration = Configuration()
        self.local_vars_configuration = local_vars_configuration

        self._ipv4_endpoint_address = None
        self._ipv6_endpoint_address = None
        self._amf_name = None
        self.discriminator = None

        if ipv4_endpoint_address is not None:
            self.ipv4_endpoint_address = ipv4_endpoint_address
        if ipv6_endpoint_address is not None:
            self.ipv6_endpoint_address = ipv6_endpoint_address
        if amf_name is not None:
            self.amf_name = amf_name

    @property
    def ipv4_endpoint_address(self):
        """Gets the ipv4_endpoint_address of this N2InterfaceAmfInfo.  # noqa: E501


        :return: The ipv4_endpoint_address of this N2InterfaceAmfInfo.  # noqa: E501
        :rtype: list[str]
        """
        return self._ipv4_endpoint_address

    @ipv4_endpoint_address.setter
    def ipv4_endpoint_address(self, ipv4_endpoint_address):
        """Sets the ipv4_endpoint_address of this N2InterfaceAmfInfo.


        :param ipv4_endpoint_address: The ipv4_endpoint_address of this N2InterfaceAmfInfo.  # noqa: E501
        :type: list[str]
        """

        self._ipv4_endpoint_address = ipv4_endpoint_address

    @property
    def ipv6_endpoint_address(self):
        """Gets the ipv6_endpoint_address of this N2InterfaceAmfInfo.  # noqa: E501


        :return: The ipv6_endpoint_address of this N2InterfaceAmfInfo.  # noqa: E501
        :rtype: list[Ipv6Addr]
        """
        return self._ipv6_endpoint_address

    @ipv6_endpoint_address.setter
    def ipv6_endpoint_address(self, ipv6_endpoint_address):
        """Sets the ipv6_endpoint_address of this N2InterfaceAmfInfo.


        :param ipv6_endpoint_address: The ipv6_endpoint_address of this N2InterfaceAmfInfo.  # noqa: E501
        :type: list[Ipv6Addr]
        """

        self._ipv6_endpoint_address = ipv6_endpoint_address

    @property
    def amf_name(self):
        """Gets the amf_name of this N2InterfaceAmfInfo.  # noqa: E501


        :return: The amf_name of this N2InterfaceAmfInfo.  # noqa: E501
        :rtype: str
        """
        return self._amf_name

    @amf_name.setter
    def amf_name(self, amf_name):
        """Sets the amf_name of this N2InterfaceAmfInfo.


        :param amf_name: The amf_name of this N2InterfaceAmfInfo.  # noqa: E501
        :type: str
        """

        self._amf_name = amf_name

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = {}

        for attr, _ in six.iteritems(self.openapi_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        return pprint.pformat(self.to_dict())

    def __repr__(self):
        """For `print` and `pprint`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, N2InterfaceAmfInfo):
            return False

        return self.to_dict() == other.to_dict()

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        if not isinstance(other, N2InterfaceAmfInfo):
            return True

        return self.to_dict() != other.to_dict()
